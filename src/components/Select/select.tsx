...
export const Select:FC<SelectProps> = (props) => {
  ...
  return (
    <div className={containerClass} ref={containerRef}>
      <div className="rf-select-input" onClick={handleClick}>
        <Input
          ref={input}
      ...
      <SelectContext.Provider value={passedContext}>
        <Transition
            ...
            timeout={300}
          >
          <ul className="rf-select-dropdown">
            {generateOptions()}
          </ul>
        </Transition>
      </SelectContext.Provider>
      {multiple &&
        <div className="rf-selected-tags" style={{maxWidth: containerWidth.current - 32}}> 
          {
            selectedValues.map((value, index) => {
              return (
                <span className="rf-tag" key={`tag-${index}`}>
                  {value}
